...
for kv = 1:nim
    ...
    for imeth = 1:length(methods)
        ...
        if imeth == length(methods)
            title(sprintf('\\rm%s (\\bf%2.2f\\rm dB, \\bf%.4f\\rm)',methnames{imeth},impsnr,imssim));
        else
            title(sprintf('\\rm%s (%2.2f dB, %.4f)',methnames{imeth},impsnr,imssim));
        end
    end
